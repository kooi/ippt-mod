    ...
    .always(
      function () {
        ...
        this.slideit = function () {
          ...
          var tooltip =
            '<div class="sltooltip"><div class="sltooltip-inner">' +
            curVal +
            '</div><div class="sltooltip-arrow"></div></div>';
...
ActiveCode.prototype.downloadFile = function (lang) {
  var fnb = this.divid;
  fnb = document.getElementById("assignment-name-content").innerText;
...
errorText.InternalErrorFix =
  "Report this error, along with your code as a bug.";
errorText.IndentationError =
...
errorText.NotImplementedErrorFix =
...
ActiveCode.prototype.fileReader = function (divid) {
  ...
  if (elem == null) {
    throw new Sk.builtin.IOError(
      "[Errno 2] No such file or directory: '" + divid + "'",
    );
  } else {
...
ActiveCode.prototype.runProg = function () {
  ...
  $(this.codeDiv).switchClass("col-md-12", "col-md-7", {
    duration: 500,
    queue: false,
  });
...
function AudioTour(divid, code, bnum, audio_text) {
  ...
  var bcount = 0;
  var html_string =
    "<div class='modal-lightsout'></div><div class='modal-profile'><h3>Take an audio tour!</h3><div class='modal-close-profile'></div><p id='windowcode'></p><p id='" +
    divid +
    "_audiocode'></p>";
  html_string += "<p id='status'></p>";
  html_string +=
    "<input type='image' src='../_static/first.png' width='25' id='first_audio' name='first_audio' title='Play first audio in tour' alt='Play first audio in tour' onerror=\"this.onerror=null;this.src='_static/first.png'\" disabled/>" +
  ...
  for (var i = 0; i < audio_type.length - 1; i++) {
    html_string +=
      "<input type='button' style='margin-right:5px;' class='btn btn-default btn-sm' id='button_audio_" +
      i +
      "' name='button_audio_" +
      i +
      "' value=" +
...
AudioTour.prototype.playaudio = function (i, aname, divid, ahash) {
  ...
  if (isNaN(this.elem.duration) || this.elem.duration == 0) {
    // set the status
    $("#status").html(
      "Loading audio.  Please wait.   If it doesn't start soon close this window (click on the red X) and try again",
    );
    $("#" + this.afile).bind(
      "canplaythrough",
      function () {
...
LiveCode.prototype.runProg = function () {
  ...
  if (this.language !== "java" || files.length === 0) {
    ...
    var instance = this;
    $.getScript(
      "http://cdn.rawgit.com/killmenot/webtoolkit.md5/master/md5.js",
      function () {
      ...
      default:
        if (result.stderr) {
          ...
          this.addJobeErrorMessage(
            "A server error occurred: " + xhr.status + " " + xhr.statusText,
          );
        }
    }
...
ACFactory.createScratchActivecode = function () {
  ...
  var html =
    ...
    "      </div> " +
    '      <div class="modal-body">' +
    '      <textarea data-component="activecode" id="' +
    divid +
    '" data-lang="' +
    lang +
    '">' +
